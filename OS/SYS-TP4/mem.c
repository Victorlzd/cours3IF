...
void * mem_alloc(int64_t length)
{
    ...
    if(size > length + 8)
    {
        void* next_block_ptr = block_ptr + length;

        assert(((size-length-8)%8)==0);

        int64_t new_size = size-length;
        char new_flags = 0b000;
        int64_t new_header = new_size | new_flags;
        *( (int64_t*)next_block_ptr ) = new_header ;
        *( (int64_t*)next_block_ptr + new_size - 8 ) = new_header ;
        size = length;
    }
...
void mem_show_heap(void)
{
    ...
    while(block_ptr < heap_end)
    {
        ...
        printf("  block at %p: header=0x%08lx size=%ld flags=%d (%s) footer = %s\n",
               block_ptr,header,size,flags,
               (flags ? "taken" : "free"),(footerOK ? "YES" : "NO")
               );
