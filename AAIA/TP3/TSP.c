...
void creeMemD(){
	...
	nbEnsemble = pow(2,nbSommets);
	memD = (int**) malloc(nbSommets*sizeof(int*));

	for(i=0; i<nbSommets; ++i)
	{
		memD[i] = (int*) malloc(nbEnsemble*sizeof(int));
...
void creeMemNext(){
    ...
    nbEnsemble = pow(2,nbSommets);
    memNext = (int**) malloc(nbSommets*sizeof(int*));

    for(i=0; i<nbSommets; ++i)
    {
        memNext[i] = (int*) malloc(nbEnsemble*sizeof(int));
        for(j=0; j<nbEnsemble; ++j)
        {
            memNext[i][j] = -1;
        }
...
int main(){
    ...
    printf("Longueur du plus court circuit hamiltonien = %d ; temps de calcul = %.3fs ; nbCalculD =%d\n",d,duree,compteurCalcul);
