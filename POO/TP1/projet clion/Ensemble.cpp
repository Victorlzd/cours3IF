...
Ensemble::Ensemble(unsigned int cardMax) : tailleMax(cardMax), tailleAct(0)
{
	#ifdef MAP
...
{
	tailleMax = unEnsemble.tailleMax;
	tailleAct = unEnsemble.tailleAct;
	tableau = new int[tailleMax];
	
	for (int i = 0; i < tailleAct; ++i)
	{
		tableau[i] = unEnsemble.tableau[i];
	}
}
